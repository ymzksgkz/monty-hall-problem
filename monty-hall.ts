...
const chooseRandom1: ChooseRandom1 = optionLength => (
  Math.floor(Math.random() * optionLength)
)
...
const generateOptions: GenerateOptions = optionLength => {
  ...
  return Array.from(
    {length: optionLength},
    (_, i) => (
      { isPlayerChoose: false, isCorrect: i === correctIndex }
    )
  )
}
...
const parseArgs: ParseArgs = () => {
  const { times, length, change } = yargs(hideBin(process.argv))
    .options('length', { alias: 'l', type: 'number', 'description': '選択肢の数', default: 3 })
    .options('times', { alias: 't', type: 'number', 'description': '試行回数', default: 100 })
    .options('change', { alias: 'c', type: 'boolean', 'description': '選択肢を変更する場合 true, 変更しない場合 false', default: true })
    .strict()
    .parseSync()
...
const main: Main = () => {
  ...
  const results = Array.from(
    { length: TIMES },
    () => doMontyHall(LENGTH, IS_CHANGE_OPTION)
  )
  // 結果が true であった割合
  const winRate = results.filter(result => result === true).length / TIMES

  console.log('=====================================================================\n')
  console.log(`条件: 選択肢の数: ${LENGTH}個, 試行回数: ${TIMES}回, 2回目の選択を行う: ${IS_CHANGE_OPTION ? '行う' : '行わない'}`)
